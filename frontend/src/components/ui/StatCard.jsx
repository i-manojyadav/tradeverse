...
function StatCard({ title, value, subTitle, isPnL, roi }) {
    ...
    return (
        <div className='stat-card'>
            ...
            <p className='stat-value' style={{ color: valueColor }}>{Number(Number(value).toFixed(2)).toLocaleString()} <span className='roi' style={{display: isPnL ? "inline-block" : "none"}}>({Number(Number(roi).toFixed(2)).toLocaleString()}%)</span></p>
...
function StatCardMobile({ invested, current, pnl, roi, isPosition }) {

    return (
        <div className='stat-card-mobile'>
            <div className='stat-value-sec'>
                <div className='stat-inv-value'>
                    <p className='stat-title-m'>{isPosition ? "Margin Used" : "Invested"}</p>
                    <p className='stat-value-m'>{Number(Number(invested).toFixed(2)).toLocaleString()}</p>
                </div>
                <div>
                    <p className='stat-title-m'>{isPosition ? "Position Value" : "Current"}</p>
